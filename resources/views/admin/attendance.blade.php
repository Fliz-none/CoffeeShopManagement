        ...
        <div class="container-fluid p-0">
            <div class="card">
                @if (!empty(Auth::user()->can(app\Models\User::CREATE_WORK)))
                    <form id="form-create-work" method="post">
                        @if (!$work || $work->status == 1)
                            <div class="card-body row text-center justify-content-center">
                                <div class="col-lg-4 col-12 remind">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="260" height="293" viewBox="0 0 586.47858 659.29778" xmlns:xlink="http://www.w3.org/1999/xlink">
                                        ...
                                        <path
                                            ...
                                            fill="#f2f2f2" />
                                        <polygon points="171.30016 646.86102 182.10017 646.85999 187.23916 605.198 171.29716 605.19897 171.30016 646.86102" fill="#a0616a" />
                                        ...
                                        <polygon points="84.74116 616.94501 93.38016 623.42603 122.49316 593.185 109.74116 583.61902 84.74116 616.94501" fill="#a0616a" />
                                        ...
                                        <path
                                            ...
                                            fill="#2f2e41" />
                                        <circle cx="281.3585" cy="285.71051" r="51.12006" transform="translate(-26.58509 542.54478) rotate(-85.26884)" fill="var(--primary)" />
                                        <path
                                        ...
                                        <g>
                                            ...
                                            <path
                                                ...
                                                fill="#3f3d56" />
                                            <path d="M456.61398,74.41416h-10.60999c-1.21002,0-2.19,.97998-2.19,2.19v10.62c0,1.21002,.97998,2.19,2.19,2.19h10.60999c1.21002,0,2.20001-.97998,2.20001-2.19v-10.62c0-1.21002-.98999-2.19-2.20001-2.19Z"
                                                fill="#3f3d56" />
                                            <path d="M430.61398,74.41416h-10.60999c-1.21002,0-2.19,.97998-2.19,2.19v10.62c0,1.21002,.97998,2.19,2.19,2.19h10.60999c1.21002,0,2.20001-.97998,2.20001-2.19v-10.62c0-1.21002-.98999-2.19-2.20001-2.19Z"
                                                fill="#3f3d56" />
                                            <path d="M481.11398,74.91416h-10.60999c-1.21002,0-2.19,.97998-2.19,2.19v10.62c0,1.21002,.97998,2.19,2.19,2.19h10.60999c1.21002,0,2.20001-.97998,2.20001-2.19v-10.62c0-1.21002-.98999-2.19-2.20001-2.19Z"
                                                fill="#3f3d56" />
                                            <path d="M321.19229,78.95414h-84.81c-1.48004,0-2.67004,1.20001-2.67004,2.67004s1.19,2.66998,2.67004,2.66998h84.81c1.46997,0,2.66998-1.20001,2.66998-2.66998s-1.20001-2.67004-2.66998-2.67004Z"
                                                fill="var(--primary)" />
                                        </g>
                                    </svg>
                                    <h5 class="my-3">Vui lòng chấm công</h5>
                                </div>
                                <div class="camera"></div>
                                <div class="image-taken d-none"></div>
                                <div class="duration text-center d-none">
                                    <p class="text-secondary">Bạn đã làm việc được</p>
                                    <div class="clock fs-4 fw-bold mt-3"></div>
                                </div>
                            </div>
                            <div class="card-footer border-0 d-flex justify-content-center">
                                <input name="status" type="hidden" value="0">
                                <button class="btn btn-primary btn-camera" type="button">Chấm công</button>
                                <button class="btn btn-primary btn-taken d-none" type="button">Chụp hình</button>
                                <button class="btn btn-primary btn-create-work d-none" type="button">Xác nhận</button>
                            </div>
                        @else
                            ...
                            <div class="card-footer border-0 d-flex justify-content-center">
                                ...
                                <button class="btn btn-primary btn-camera" type="button">Check out</button>
                                <button class="btn btn-primary btn-taken d-none" type="button">Chụp hình</button>
                                <button class="btn btn-primary btn-create-work d-none" type="button">Xác nhận</button>
                            </div>
                        @endif
...
@push('scripts')
    ...
    <script>
        ...
        $('.btn-taken').click(function(e) {
            e.preventDefault()
            // play sound effect
            sound.play();
            $(this).addClass('d-none').next().removeClass('d-none')
            $('.camera').addClass('d-none').next().removeClass('d-none');
            // take snapshot and get image data
            Webcam.snap(function(data_uri) {
                $('.image-taken').html('<img id="imageprev" src="' + data_uri + '"/>')
            });
            Webcam.reset();
        })

        $('.btn-create-work').click(function(e) {
            e.preventDefault()
            $(this).addClass('d-none')
            $('.image-taken').addClass('d-none').next().removeClass('d-none');
            var element = $('.image-taken'),
                frm = $('#form-create-work'),
                status = frm.find(`input[name='status']`).val()
            html2canvas(element, {
                background: '#ffffff',
                onrendered: function(canvas) {
                    var imgData = document.getElementById("imageprev").src;
                    $.ajax({
                        url: `{{ url('work/create') }}`,
                        type: 'post',
                        dataType: 'text',
                        ...
                        success: function(response) {
                            ...
                            if (json_response.obj && json_response.obj.status == 0) {
                                setInterval(() => {
                                    showTime(json_response.obj.created_at)
                                }, 1000);
                                $('.btn-camera').text('Check out').removeClass('d-none');
                            } else {
                                $('.duration').html('<span class="text-primary fw-bold">' + json_response.title +
                                    '</span><br/><a href="#" onClick="window.location.reload();" class="btn btn-primary mt-3">Chấm công mới</a>')
                            }
                            Toastify({
                                ...
                                gravity: "top",
                                position: "center",
                                backgroundColor: `var(--bs-'${json_response.status}')`,
                            }).showToast();
                        },
                    });
                }
